...
###
# Description :
# This code is for automatically running our Performance Tool (Google Chrome) with reading 100 websites (from website_list.txt) and Noise Template (data)
# Run (1) prime_probe_Test.html to perform JavaScript-base Prime and Probe attack.
# (2) Load Noise Template create by SMC (data)
# (3) PerformanceTool_GC_PnP_Noise.py by inputing website address ("$line")
# (4) Close all taps
#
# Make sure chrome driver is properly installed while runing PerformanceTool_GC_PnP_Noise.py code
#
...
do
  ...
  do
    ...
    data="Noise_Template_repeat_sleep/repeat_sleep_second_try_"$i"_"$m".csv"
    ...
    #killall prog_800
    python3 PerformanceTool_GC_PnP_Noise.py "$line" &
    sleep 3s
    #echo "heyy"
    start=$(date +%s)
    for ((j=0;j < 15;j++));
